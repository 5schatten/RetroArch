...
bool recording_init(void)
{
   ...
   else
   {
      ...
      else
      {
         ...
         if (settings->uints.video_record_quality < RECORD_CONFIG_TYPE_RECORDING_WEBM_VP8_FAST)
         {
            fill_str_dated_filename(buf, game_name,
                     "mkv", sizeof(buf));
            fill_pathname_join(output, global->record.output_dir, buf, sizeof(output));
         }
         else if (settings->uints.video_record_quality >= RECORD_CONFIG_TYPE_RECORDING_WEBM_VP8_FAST && settings->uints.video_record_quality < RECORD_CONFIG_TYPE_RECORDING_WEBM_GIF)
         {
            fill_str_dated_filename(buf, game_name,
                     "webm", sizeof(buf));
            fill_pathname_join(output, global->record.output_dir, buf, sizeof(output));
